...
TXDrawFractalDelay3D::TXDrawFractalDelay3D() {
    setup();
}

void TXDrawFractalDelay3D::setup(){
...
void TXDrawFractalDelay3D::drawFractalBranch(int levelArg) {
    float holdOffsetX, holdOffsetY, holdOffsetZ;
    float holdScaleX, holdScaleY, holdScaleZ;
    float holdAngle, holdRotateAxisX, holdRotateAxisY, holdRotateAxisZ, holdLevelDelayFrames;
    float holdBranchRandom, holdSubBranchRandom, holdAngleRandom, holdScaleXRandom, holdScaleYRandom, holdScaleZRandom, holdDelayRandom;
    bool  makeBranch;
    ...
    float holdScaleZDelay;
    float holdScaleZLevelDeltaDelay;
    float holdScaleZRandomDelay;
        ...
        for (int branchNo = 0; branchNo < fractalTreeBranches->getInt(); branchNo++){
            ...
            if (makeBranch && ((arrFractalBranches[branchNo].branchProbability->getFloat() + holdBranchRandom) >= 1) )  {
                ...
                */
                ...
                holdOffsetZ = arrFractalBranches[branchNo].getDelayed_offsetZ(holdLevelDelayFrames) * cellDepth->getFloat() * getMaxDepth();
                holdRotateAxisX = arrFractalBranches[branchNo].getDelayed_rotateAxisX(holdLevelDelayFrames) ;
                holdRotateAxisY = arrFractalBranches[branchNo].getDelayed_rotateAxisY(holdLevelDelayFrames) ;
                holdRotateAxisZ = arrFractalBranches[branchNo].getDelayed_rotateAxisZ(holdLevelDelayFrames) ;
                ...
                holdScaleZDelay = arrFractalBranches[branchNo].getDelayed_scaleZ(holdLevelDelayFrames);
                holdScaleZLevelDeltaDelay = arrFractalBranches[branchNo].getDelayed_scaleZLevelDelta(holdLevelDelayFrames);
                holdScaleZRandomDelay = arrFractalBranches[branchNo].getDelayed_scaleZRandom(holdLevelDelayFrames);
                ...
                if (arrFractalBranches[branchNo].useScaleXForScaleZ) {
                    ...
                    holdScaleZ = holdScaleZDelay + (levelArg * holdScaleZLevelDeltaDelay) + (holdScaleZRandomDelay * holdScaleZRandom);
                };
                ...
                holdAnchorZ = getMaxDepth() * arrFractalBranches[branchNo].getDelayed_anchorZ(holdLevelDelayFrames);
