...
void main() {
 vec2 q;
 vec2 r;
 vec2 c = zoom * 300.0 * gl_FragCoord.xy / resolution.xy;
 float f = pattern( c * 0.01, q, r);
 vec4 col = mix( color1, color2, clamp( (f*f) * 4.0,0.0,1.0));
 //col = color2;
 col = mix(col,color3,clamp(length(q),0.0,1.0));
 col = mix(col,color4,clamp(length(r.x),0.0,1.0));
 gl_FragColor =  vec4((0.2*f*f*f+0.6*f*f+0.5*f)*col.xyz,col.a);
}