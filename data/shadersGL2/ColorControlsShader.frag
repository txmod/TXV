...
void main() 
{
	vec4 t0 = texture2D(u_texture0Sampler, v_texCoord0.st);
	vec3 bcs = BrightnessContrastSaturation(t0.rgb, u_brightness, u_contrast, u_saturation);

	vec4 color = vec4(bcs, t0.a);
	gl_FragColor = color;
}
