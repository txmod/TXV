...
void main(void)
{
	...
	for (int i=0; i < 3; ++i)
	{
		for (int j=0; j < 3; ++j)
		{
//            color = texture2DRect(u_texture0Sampler, (gl_FragCoord.xy+vec2(i-1,j-1)));
            color = texture2DRect(u_texture0Sampler, (v_texCoord0.xy+vec2(i-1,j-1)));
			c[3*i+j] = color.rgb;
            if ((i == 1) && (j == 1)) {
                tex0 = color; // middle color
            };            
		}
	}
