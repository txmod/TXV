...
varying vec2 v_normCoord0; // 2nd texture stored in normal
...

{
    ...
    if ((v_texCoord0.x < 1) || (v_texCoord0.x > (u_texdim0.x - 1)) || (v_texCoord0.y < 1) || (v_texCoord0.y > (u_texdim0.y - 1)) ) {
        tex0 = black;
    } else {
        tex0 = texture2DRect(u_texture0Sampler, v_texCoord0);
    };
    if ((v_normCoord0.x < 1) || (v_normCoord0.x > (u_texdim1.x - 1)) || (v_normCoord0.y < 1) || (v_normCoord0.y > (u_texdim1.y - 1)) ) {
        tex1 = black;
    } else {
        tex1 = texture2DRect(u_texture1Sampler, v_normCoord0);
    };
