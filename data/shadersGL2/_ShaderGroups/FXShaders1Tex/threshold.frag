...
uniform vec2 aboveSamplePos;
uniform vec2 belowSamplePos;
uniform int useSampledAboveColor;
uniform int useSampledBelowColor;
...
void main(){
    vec4 fxColor;
    vec4 tex0 = texture2DRect(u_texture0Sampler, gl_TexCoord[0].st);
    float average = (tex0.r + tex0.g + tex0.b) / 3.0;
    if (average >= threshold) {
        if (useSampledAboveColor == 1) {
            fxColor = texture2DRect(u_texture0Sampler, aboveSamplePos);
        } else {
            fxColor = aboveColor;
        };
    } else {
        if (useSampledBelowColor == 1) {
            fxColor = texture2DRect(u_texture0Sampler, belowSamplePos);
        } else {
            fxColor = belowColor;
        };
    };
