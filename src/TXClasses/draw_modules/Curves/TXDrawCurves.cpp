...
TXDrawCurves::TXDrawCurves() {
    setup();
}

void TXDrawCurves::setup(){
    ...
    holdModParamBoolArgs.name = "ifSingleIgnoreEndValues";
    holdModParamBoolArgs.fixedValue = false;
    holdModParamBoolArgs.fixedModMix = false;
    ifSingleIgnoreEndValues = createModParameter(holdModParamBoolArgs);
    parameters.add(ifSingleIgnoreEndValues->parameters);
...
void TXDrawCurves::draw(){
    ...
    bool holdIfSingleIgnoreEndValues = ifSingleIgnoreEndValues->getBool();
    ...
    for (int curveIndex = 0; curveIndex < totalCurves; curveIndex++){
        ...
        if (totalCurves == 1) {
            if (holdIfSingleIgnoreEndValues) {
                proportion = 0.0;
            } else {
                proportion = 0.5;
            };
       ...
       for (int pointNo = 0; pointNo < totalPoints; pointNo++){
           arrdrawX[pointNo] = getMaxWidth() * arrCurvePoints[pointNo].getInterpDrawX(positionSpreadProportion);
            arrdrawY[pointNo] = getMaxHeight() * arrCurvePoints[pointNo].getInterpDrawY(positionSpreadProportion);
            
            if ((pointNo > 0) && isBezier ) {
                arrcontrol1X[pointNo] = getMaxWidth() * arrCurvePoints[pointNo].getInterpControl1X(positionSpreadProportion);
                arrcontrol1Y[pointNo] = getMaxHeight() * arrCurvePoints[pointNo].getInterpControl1Y(positionSpreadProportion);
                arrcontrol2X[pointNo] = getMaxWidth() * arrCurvePoints[pointNo].getInterpControl2X(positionSpreadProportion);
                arrcontrol2Y[pointNo] = getMaxHeight() * arrCurvePoints[pointNo].getInterpControl2Y(positionSpreadProportion);
            };
        ...
        if (isAutoCurve) {
            if (isAutoClose || isFillShape) {
                ...
                if (usePoint1ForAutoCurveStart) {
                    ...
                    autoCurveStartX = getMaxWidth() * autoCurveStart.getInterpControl1X(positionSpreadProportion);
                    autoCurveStartY = getMaxHeight() * autoCurveStart.getInterpControl1Y(positionSpreadProportion);
                };
                if (useLastPointForAutoCurveEnd) {
                    ...
                    autoCurveEnd1X = getMaxWidth() * autoCurveEnd.getInterpControl1X(positionSpreadProportion);
                    autoCurveEnd1Y = getMaxHeight() * autoCurveEnd.getInterpControl1Y(positionSpreadProportion);
                    autoCurveEnd2X = autoCurveEnd1X;
                    autoCurveEnd2Y = autoCurveEnd1Y;
                };
            };
