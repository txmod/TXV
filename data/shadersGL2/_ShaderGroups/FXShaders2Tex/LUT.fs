...
//	float x = tex0.r * (size-1.0); incorrect - this produces blue blotches
//	float y = tex0.g * (size-1.0);
//	float z = tex0.b * (size-1.0);
    float x = tex0.r * (size-2.0);
    float y = tex0.g * (size-2.0);
    float z = tex0.b * (size-2.0);

    // LUT
   fxColor = texture2DRect(u_texture1Sampler, vec2(floor(x)+0.5 +(floor(y)*size+0.5), floor(z)+0.5) ).rgb;
